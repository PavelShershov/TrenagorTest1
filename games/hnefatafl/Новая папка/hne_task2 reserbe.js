...
(() => {
    ...
    function generateEarlyPosition() {
        const steps = Math.floor(Math.random() * 12) + 4;
    ...
    function generateLongLinePosition(target) {
        for (let attempt = 0; attempt < 25; attempt++) {
            ...
            for (let step = 1; step <= target; step++) {
                ...
                if (FORTRESSES.has(ncell) || ncell === THRONE) {
                    ok = false;
                    break;
                }
            ...
            for (let i = 1; i <= 121; i++) {
                if (!pos[i] && i !== THRONE && !FORTRESSES.has(i)) {
                    allCells.push(i);
                }
            }
    ...
    function makeOptions(correct) {
        ...
        while (answers.length < 4) {
            ...
            if (next >= 1 && next <= 8 && !answers.includes(next)) {
                answers.push(next);
            } else {
                const fallback = Math.max(1, Math.min(8, answers.length + 1));
    ...
    function generateMaxCaptureTask() {
        ...
        for (let attempt = 0; attempt < 100; attempt++) {
            ...
            const prob = (weights[maxCap] || 1) / maxWeight;
        ...
        for (let attempt = 0; attempt < 300; attempt++) {
            ...
            if (maxCap >= 1 && maxCap <= 7 && bestMoves.length) {
                return buildTaskFromPosition(pos, maxCap, bestMoves);
            }
        }
