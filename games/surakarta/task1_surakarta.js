...
// (с новым форматом пояснений: взаимная рубка помечается в конце)
...
(() => {
    ...
    // Новая функция построения пояснения
    function buildExplanation(allCaptures, board) {
        ...
        const attackedMap = new Map(); // target -> [{attackerPos, attackerColor}]
        const lines = [];
        const printedMutual = new Set();

        // Обрабатываем взаимные атаки
        for (const [color, start, target] of allCaptures) {
            ...
            if (mutualPairs.has(pairKey) && !printedMutual.has(pairKey)) {
                ...
                // Запоминаем, что обе фишки атакованы
        ...
        for (const [color, start, target] of allCaptures) {
            ...
            const defenderName = defenderColor === 'P' ? 'фиолетовую' : 'серую'; // винительный падеж

            const existingAttackers = attackedMap.get(target);
            if (!existingAttackers || existingAttackers.length === 0) {
                // Первая атака на эту фишку
                if (!attackedMap.has(target)) attackedMap.set(target, []);
